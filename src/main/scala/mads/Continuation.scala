...
object continuation {
  opaque type Continuation[S, A, B] = (ctrl: Control) => Result[A] => ctrl.F[S, B]
  object Continuation {
    def apply[S, A, B](f: (ctrl: Control) => Result[A] => ctrl.F[S, B]): Continuation[S, A, B] =
      f
  }
  ...
  extension [S, A, B](cont: Continuation[S, A, B]) {
    ...
    def map[C](f: B => C): Continuation[S, A, C] =
      Continuation((ctrl: Control) => (r: Result[A]) => ctrl.map(cont(ctrl)(r))(f))

    def contramap[C](f: C => A): Continuation[S, C, B] =
      Continuation((ctrl: Control) => (r: Result[C]) =>
        r match {
